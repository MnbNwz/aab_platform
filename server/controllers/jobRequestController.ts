...
const ALLOWED_CATEGORIES = [
  "painting", "plumbing", "electrical", "cleaning", "renovation", "hvac", "other"
];
...
export const createJobRequest = async (req: Request, res: Response) => {
  try {
    ...
    if (type !== "off_market" && !property) {
      return res.status(400).json({ success: false, message: "Property is required for regular jobs" });
    }
    // Only admin can create off_market jobs
    if (type === "off_market" && authReq.user.role !== "admin") {
      return res.status(403).json({ success: false, message: "Only admin can create off-market jobs" });
    }
...
export const getJobRequests = async (req: Request, res: Response) => {
  try {
    ...
    let jobs;
    if (user.role === "admin") {
      jobs = await JobRequest.find().populate("createdBy property assignedContractor acceptedBid bids");
      ...
      jobs = await JobRequest.find({ createdBy: user._id }).populate("property assignedContractor acceptedBid bids");
    }
...
export const getJobRequestById = async (req: Request, res: Response) => {
  try {
    const job = await JobRequest.findById(req.params.id).populate("createdBy property assignedContractor acceptedBid bids");
...
export const updateJobRequest = async (req: Request, res: Response) => {
  try {
    ...
    for (const key of Object.keys(req.body)) {
      ...
      // Validate updates
      if (key === "title" && (typeof req.body.title !== "string" || req.body.title.length < 5 || req.body.title.length > 100)) {
        return res.status(400).json({ success: false, message: "Title must be 5-100 characters" });
      }
      if (key === "description" && (typeof req.body.description !== "string" || req.body.description.length < 10 || req.body.description.length > 2000)) {
        return res.status(400).json({ success: false, message: "Description must be 10-2000 characters" });
      }
      if (key === "category" && req.body.category && !ALLOWED_CATEGORIES.includes(req.body.category)) {
        return res.status(400).json({ success: false, message: "Invalid category" });
      }
    ...
    if (req.body.status) {
      if (req.body.status === "completed" && user.role !== "admin" && String(job.assignedContractor) !== String(user._id)) {
        return res.status(403).json({ success: false, message: "Only assigned contractor or admin can mark as completed" });
      }
    }
    await job.save();
    res.json({ success: true, job });
